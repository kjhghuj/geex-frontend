...
export async function generateMetadata({ searchParams }: ShopPageProps): Promise<Metadata> {
  ...
  const baseUrl = process.env.NEXT_PUBLIC_BASE_URL || "https://www.geexfans.com";

  const title = category
    ? `Shop ${category.charAt(0).toUpperCase() + category.slice(1).replace("-", " ")} | GEEX`
    : "Shop Electronics & Setup Gear | GEEX";

  const description = category
    ? `Explore GEEX ${category} products for cleaner workspaces, gaming stations, and mobile setups.`
    : "Discover GEEX keyboards, gaming peripherals, mobile accessories, charging gear, and Bluetooth audio.";
