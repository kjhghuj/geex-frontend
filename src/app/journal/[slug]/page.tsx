...
export async function generateMetadata({
  ...
  if (!article) {
    return { title: "Article Not Found | GEEX Journal" };
  }

  return {
    title: `${article.title} | GEEX Journal`,
...
const StickySidebar = ({ product }: { product: StoreProduct | null | undefined }) => {
  ...
  const category = product.categories?.[0]?.name || "Setup Gear";
    ...
    <div className="sticky top-32 w-full animate-fade-in-up">
      <div className="bg-white border border-gray-100 p-5 rounded-sm shadow-sm">
        <div className="flex items-center justify-between mb-4 pb-2 border-b border-gray-50">
          <span className="text-[9px] uppercase tracking-[0.2em] text-orbit-blue font-bold">
            Featured Item
          </span>
        </div>
        ...
        <Link href={`/product/${product.handle}`} className="group block">
          ...
          <div className="flex justify-between items-start mb-4">
            <div>
              <h4 className="font-serif text-lg text-near-black group-hover:text-orbit-blue transition-colors leading-tight">
                {product.title}
              </h4>
            ...
            <div className="text-right">
              <p className="text-orbit-blue font-bold text-sm">
                {price ? `￡${price.toFixed(2)}` : ""}
              </p>
            </div>
          </div>

          <button className="w-full bg-near-black text-white py-3 text-[10px] uppercase tracking-[0.2em] font-bold hover:bg-orbit-blue transition-colors shadow-sm">
            View Details
          </button>
        </Link>
      </div>
      ...
      {content.map((block, idx) => {
        if (block.type === "paragraph") {
          ...
          return (
            <div
              key={idx}
              className={`text-lg md:text-xl font-light leading-[1.8] text-ink-muted ${isFirst ? "flow-root" : ""
                }`}
            >
              {isFirst && block.text ? (
                <span className="float-left mr-3 text-6xl font-serif text-orbit-blue leading-[0.8] pt-2">
                  {block.text.charAt(0)}
                </span>
              ) : null}
        ...
        if (block.type === "blockquote") {
          return (
            <blockquote
              key={idx}
              className="my-12 p-8 bg-[#F9F8F6] border-l-4 border-orbit-blue rounded-r-sm"
            >
              <p className="font-serif text-2xl md:text-3xl italic text-near-black leading-snug">
                &quot;{block.text}&quot;
              </p>
            </blockquote>
          );
        ...
        if (block.type === "heading") {
          const className = "font-serif text-2xl md:text-3xl text-near-black mt-12 mb-6";
        ...
        if (block.type === "list" && block.items) {
          return (
            <ul
              key={idx}
              className="list-disc list-inside space-y-2 text-lg text-ink-muted font-light ml-4"
            >
    ...
    <div className="bg-cool-white min-h-screen pt-[72px] lg:pt-[88px]">
      ...
      <header className="max-w-[1000px] mx-auto px-6 lg:px-8 pt-12 lg:pt-24 pb-12 text-center relative z-10">
        <div className="text-[10px] uppercase tracking-widest text-gray-400 mb-8 flex items-center justify-center gap-2">
          <Link
            href="/journal"
            className="hover:text-orbit-blue transition-colors"
          >
            Journal
          </Link>
          <span className="text-gray-300">/</span>
          <span className="text-near-black">{article.category}</span>
        </div>

        <h1 className="font-serif text-3xl md:text-5xl lg:text-6xl text-near-black mb-8 leading-[1.15] tracking-tight">
          {article.title}
        </h1>

        <div className="flex flex-wrap items-center justify-center gap-4 text-xs font-bold uppercase tracking-widest text-ink-muted">
          <span>{article.date || "Oct 24, 2023"}</span>
          <span className="w-1 h-1 bg-orbit-blue rounded-full"></span>
      ...
      <div className="max-w-[1200px] mx-auto px-6 lg:px-8 grid grid-cols-1 lg:grid-cols-12 gap-12 mb-24 relative z-0 items-start">
        ...
        <div className="lg:col-span-9 lg:pr-12 flex flex-col min-w-0">
          ...
          <div className="flex gap-6 mb-12 text-gray-400 border-b border-gray-100 pb-6">
            <span className="text-xs uppercase tracking-widest font-bold text-near-black self-center mr-auto">
              Share Story
            </span>
            <Share2
              size={18}
              className="hover:text-orbit-blue cursor-pointer transition-colors"
            />
            <Facebook
              size={18}
              className="hover:text-orbit-blue cursor-pointer transition-colors"
          ...
          <div className="font-sans text-ink-muted space-y-8 flex-1">
          ...
          <div className="mt-16 pt-8 border-t border-gray-200">
            <h4 className="font-serif text-lg text-near-black mb-2">
              About The Editorial Team
            </h4>
            <p className="text-sm font-light text-ink-muted leading-relaxed">
              The GEEX editorial team writes practical guides for desk setups, peripherals, mobile accessories, and everyday electronics.
            </p>
          </div>
        </div>
        ...
        <section className="bg-white py-24 border-t border-gray-100 relative z-10">
          <div className="max-w-[1000px] mx-auto px-6 lg:px-8">
            <div className="text-center mb-16">
              <span className="text-xs font-bold uppercase tracking-widest text-orbit-blue mb-4 block">
                Up Next
              </span>
              <h2 className="font-serif text-3xl md:text-4xl text-near-black">
                Continue Reading
              </h2>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
              {relatedArticles.map((relArticle) => (
                ...
                >
                  ...
                  <h3 className="font-serif text-2xl text-near-black group-hover:text-orbit-blue transition-colors leading-tight">
                    {relArticle.title}
                  </h3>
                </Link>
              ))}
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            publisher: {
              ...
              logo: {
                "@type": "ImageObject",
                url: `${process.env.NEXT_PUBLIC_BASE_URL || "https://www.geexfans.com"}/brand/geex-logo-lockup.png`,
              },
            },
            description: article.excerpt,
          }),
