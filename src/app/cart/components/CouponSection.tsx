...
export default function CouponSection({
  ...
  return (
    <div className="border border-gray-200 rounded-xl overflow-hidden mb-4">
      ...
      >
        <div className="flex items-center gap-2 text-near-black font-medium">
          <CouponIcon />
          <span>Have a promo code?</span>
        </div>
        <span className={`text-ink-muted transition-transform duration-200 ${isOpen ? 'rotate-180' : ''}`}>
        ...
        <div className="p-4 bg-gray-50 border-t border-gray-200">
                ...
                return (
                  <div key={code} className="flex items-center justify-between bg-white px-3 py-2 rounded-lg border border-green-100 shadow-sm">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-mono font-medium text-near-black">{code}</span>
                      {discountAmount > 0 && (
          ...
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 px-4 py-2 bg-white border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-near-black/20 focus:border-near-black font-mono uppercase placeholder:normal-case"
              disabled={applying || isLoading}
            />
            <button
              onClick={handleApply}
              disabled={!inputCode.trim() || applying || isLoading}
              className="px-6 py-2 bg-near-black text-white text-sm font-medium rounded-lg hover:bg-ink-muted transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
