...
export default function OrderSummary({
  ...
  useEffect(() => {
    async function fetchOptions() {
      if (cart?.id) {
        ...
        const savedInfo = localStorage.getItem(`geex_shipping_${cart.id}`);
        if (savedInfo) {
        ...
        if (!hasExistingMethod && options.length > 0) {
          ...
          if (cheapestOption) {
            ...
            try {
              ...
              localStorage.setItem(`geex_shipping_${cart.id}`, cheapestOption.id);
              await refreshCart();
            } catch (error) {
  ...
  const handleShippingSelect = async (optionId: string) => {
    ...
    if (cart?.id) localStorage.setItem(`geex_shipping_${cart.id}`, optionId);
  ...
  return (
    <div className="bg-cool-white rounded-2xl p-6 lg:p-8 sticky top-24 shadow-sm border border-stone-100">
      <h2 className="font-serif text-xl text-near-black mb-6 flex items-center gap-2">
        Order Summary
      </h2>

      {/* Shipping Selector */}
      <div className="mb-8 relative" ref={dropdownRef}>
        <label className="block text-xs uppercase tracking-widest text-ink-muted mb-3 font-medium">Shipping Method</label>

        {loadingMethods ? (
          <div className="h-12 w-full bg-white border border-gray-200 rounded-lg flex items-center justify-center text-ink-muted gap-2 text-sm">
            <LoadingSpinner /> Loading options...
          </div>
        ) : shippingOptions.length === 0 ? (
          <div className="text-sm text-ink-muted italic p-3 bg-gray-50 rounded-lg border border-dashed border-gray-200">
          ...
          <div className="relative">
            ...
            <button
              ...
              className={`w-full flex items-center justify-between bg-white border rounded-xl px-4 py-3.5 text-left transition-all duration-200 hover:border-orbit-blue/50 focus:outline-none focus:ring-2 focus:ring-orbit-blue/10 ${isOpen ? "border-orbit-blue ring-2 ring-orbit-blue/10" : "border-gray-200"
                } ${updatingShipping ? "opacity-70 cursor-wait" : ""}`}
            >
              <div className="flex items-center gap-3 overflow-hidden">
                <div className="w-8 h-8 rounded-full bg-stone-50 flex items-center justify-center text-ink-muted shrink-0">
                  <Truck className="w-4 h-4" />
                </div>
                <div className="flex flex-col min-w-0">
                  <span className="text-sm font-medium text-near-black truncate block">
                    ...
                    return (
                      <button
                        ...
                        className={`w-full flex items-center justify-between px-3 py-2.5 rounded-lg text-sm transition-colors duration-150 ${isSelected
                          ? "bg-stone-50 text-near-black font-medium"
                          : "text-ink-muted hover:bg-stone-50 hover:text-near-black"
                          }`}
                      >
                        <div className="flex items-center gap-3">
                          <div className={`w-4 h-4 rounded-full border flex items-center justify-center shrink-0 ${isSelected ? "border-orbit-blue bg-orbit-blue" : "border-gray-300"
                            }`}>
                        ...
                        <div className="flex items-center gap-2">
                          <span className={`text-xs ${option.amount === 0 ? "text-green-600 font-medium" : "text-ink-muted"}`}>
                            {option.amount === 0 ? "Free" : formatPrice(option.amount, currencyCode)}
                          </span>
                          {isSelected && <Check className="w-3.5 h-3.5 text-orbit-blue" />}
                        </div>
                      </button>
                    );
                  })}
      ...
      <div className="space-y-3 pt-6 border-t border-gray-100">
        ...
        <div className="flex justify-between text-sm group relative">
          <span className="text-ink-muted flex items-center gap-1.5">
            Shipping
            {!selectedOption && <span className="w-1.5 h-1.5 rounded-full bg-orbit-blue animate-pulse"></span>}
          </span>
          <span className="text-near-black font-medium">
            {shipping !== null ? (
              shipping === 0 ? (
              ...
              <span className="text-ink-muted text-xs italic">Calculated at checkout</span>
            )}
          </span>
        </div>
        ...
        <div className="pt-4 mt-2">
          <div className="flex justify-between items-baseline mb-1">
            ...
            <span className="font-serif text-2xl text-near-black font-medium">
              {formatPrice(total, currencyCode)}
            </span>
          </div>
          <p className="text-xs text-right text-ink-muted">Including taxes and duties</p>
        </div>
      </div>
      ...
      <div className="mt-8 space-y-4">
        ...
        <Link href="/checkout">
          <button
            ...
            className="w-full bg-near-black text-white py-4 rounded-xl hover:bg-black transition-all duration-300 font-medium disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 shadow-lg shadow-near-black/10 hover:shadow-xl hover:shadow-near-black/20 hover:-translate-y-0.5 active:translate-y-0 active:shadow-md"
          >
        ...
        <Link
          href="/shop"
          className="block w-full text-center text-xs uppercase tracking-widest text-ink-muted hover:text-orbit-blue transition-colors py-2"
        >
          Continue Shopping
        </Link>
      </div>
      ...
      <div className="mt-8 pt-6 border-t border-gray-100 grid grid-cols-2 gap-y-3 gap-x-2">
        <div className="flex items-center gap-2 text-[10px] uppercase tracking-wide text-ink-muted">
          <div className="w-1 h-1 bg-green-500 rounded-full"></div>
          Secure checkout
        </div>
        <div className="flex items-center gap-2 text-[10px] uppercase tracking-wide text-ink-muted">
          <div className="w-1 h-1 bg-ink-muted rounded-full"></div>
          Protective packaging
        </div>
        <div className="flex items-center gap-2 text-[10px] uppercase tracking-wide text-ink-muted col-span-2">
          <div className="w-1 h-1 bg-ink-muted rounded-full"></div>
          Reliable electronics support
        </div>
      </div>
    </div>
  );
