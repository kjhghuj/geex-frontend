...
export function CouponsContent({ user }: { user: any }) {
  ...
  return (
    <div className="bg-white rounded-lg p-6 md:p-8 border border-gray-100">
      <h2 className="font-serif text-xl md:text-2xl text-near-black mb-6">My Coupons</h2>

      {/* Redeem Section */}
      <div className="mb-8 md:mb-10 bg-gray-50 p-5 md:p-6 rounded-xl border border-gray-200">
        <h3 className="font-medium text-near-black mb-4 text-sm md:text-base">Redeem New Coupon</h3>
        <div className="flex flex-col sm:flex-row gap-3">
          <input
            ...
            className="flex-1 border border-gray-200 px-4 py-3 rounded-lg focus:outline-none focus:border-orbit-blue uppercase font-mono text-sm"
          />
          <button
            onClick={handleRedeem}
            disabled={!couponCode.trim() || redeeming}
            className="w-full sm:w-auto bg-orbit-blue text-white px-6 py-3 rounded-lg uppercase tracking-widest text-xs font-bold hover:bg-blue-hover transition-colors disabled:opacity-50 flex items-center justify-center gap-2 whitespace-nowrap"
          >
      ...
      {/* Coupons List */}
      <div>
        <h3 className="font-medium text-near-black mb-4 text-sm md:text-base">Your Wallet</h3>
        {collectedCoupons.length === 0 ? (
          <div className="text-center py-10 text-ink-muted bg-gray-50/50 rounded-lg border border-dashed border-gray-200">
            ...
            {collectedCoupons.map((code) => (
              <div key={code} className="group relative bg-white border border-gray-200 rounded-lg overflow-hidden flex shadow-sm transition-all hover:shadow-md">
                ...
                <div className="flex-1 p-4 flex flex-col justify-center border-r border-dashed border-gray-200 relative">
                  ...
                  <span className="text-[10px] text-ink-muted uppercase tracking-wider mb-1">Coupon Code</span>
                  <span className="font-mono text-lg md:text-xl font-bold text-orbit-blue break-all">{code}</span>
                </div>
                ...
                <div className="bg-gray-50 p-0 w-14 flex items-center justify-center border-l-0">
                  <button
                    ...
                    className="w-full h-full flex items-center justify-center text-ink-muted hover:text-near-black hover:bg-gray-100 transition-colors"
                    title="Copy Code"
                  >
                    {copiedCode === code ? (
