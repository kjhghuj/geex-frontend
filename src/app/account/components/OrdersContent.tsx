...
export function OrdersContent({ user }: { user: any }) {
  ...
  return (
    <div className="bg-white rounded-lg p-6 md:p-8 border border-gray-100 min-h-[400px]">
      <h2 className="font-serif text-xl md:text-2xl text-near-black mb-6">Order History</h2>
                  ...
                  <div className="flex items-center gap-4 flex-1 min-w-0">
                    ...
                    <div className="w-16 h-16 bg-white rounded-lg border border-gray-200 flex-shrink-0 overflow-hidden relative">
                      ...
                      {order.items.length > 1 && (
                        <div className="absolute bottom-0 right-0 bg-near-black/80 text-white text-[9px] px-1.5 py-0.5 rounded-tl-md">
                          +{order.items.length - 1}
                        </div>
                      )}
                    </div>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="font-medium text-near-black text-xs uppercase">Order: {order.id.replace("order_", "").slice(0, 10)}...</span>
                      ...
                      <div className="flex gap-3 text-xs text-ink-muted">
                        ...
                        <span>/</span>
                        <span className="font-medium text-near-black">{formatPrice(order.total, order.currency_code)}</span>
                      </div>
                    </div>
                  </div>
                ...
                {isExpanded && order.items && order.items.length > 0 && (
                  <div className="border-t border-gray-100 p-4 bg-white animate-accordion-down">
                      ...
                      {order.items.map((item) => {
                        ...
                        return (
                          <div key={item.id} className="flex items-start gap-4">
                            ...
                            <div className="flex-1 min-w-0">
                              <p className="text-sm font-medium text-near-black truncate">{item.title}</p>
                              <p className="text-xs text-ink-muted mb-1">{item.variant_title}</p>
                              <div className="flex justify-between items-center text-xs">
                                <span className="text-ink-muted">Qty: {item.quantity}</span>
                                <span className="font-medium text-near-black">{formatPrice(item.unit_price, order.currency_code)}</span>
                              </div>
                            </div>
                          </div>
                        );
                    ...
                    <div className="mt-4 pt-4 border-t border-gray-100 flex justify-between items-center">
                      <Link href={`/order/lookup?order=${order.id}&email=${encodeURIComponent(order.email || user?.email)}`} className="text-xs font-bold uppercase tracking-widest text-orbit-blue hover:text-blue-hover">
                        View Full Details
                      </Link>
                      <div className="text-right">
                        <span className="text-xs text-ink-muted">Total: </span>
                        <span className="font-serif text-lg font-bold text-near-black">{formatPrice(order.total, order.currency_code)}</span>
                      </div>
                    </div>
                  </div>
                )}
