...
export default function ProductActions({ product, onVariantChange }: ProductActionsProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      {options.map((option) => (
        <div key={option.id}>
          <h3 className="text-xs uppercase tracking-widest text-near-black mb-3 flex items-center justify-between">
              ...
              return (
                <button
                  ...
                  className={`px-4 py-2.5 text-sm border rounded-md transition-all ${isSelected
                    ...
                    : hasStock
                      ? "border-gray-200 text-near-black hover:border-near-black"
      ...
      {variants.length > 1 && price && (
        <div className="flex items-baseline gap-3 pt-2">
          <span className="text-xl font-medium text-near-black">
      ...
      {/* Quantity Selector */}
      <div>
        <h3 className="text-xs uppercase tracking-widest text-near-black mb-3">
          Quantity
        </h3>
      ...
      <div className="space-y-3">
        ...
        <button
          ...
          className={`w-full py-4 rounded-full uppercase tracking-widest text-sm font-bold transition-all flex items-center justify-center gap-2 ${justAdded
            ...
            : !isAllOptionsSelected
              ? "bg-gray-100 text-ink-muted border border-gray-200 cursor-default"
              : isInStock
                ? "bg-orbit-blue text-white hover:bg-blue-hover disabled:opacity-50 disabled:cursor-not-allowed"
        ...
        <button
          ...
          className={`w-full py-4 rounded-full uppercase tracking-widest text-sm font-bold transition-all flex items-center justify-center gap-2 ${!isAllOptionsSelected
            ? "bg-gray-50 text-ink-muted border border-gray-100 cursor-default opacity-50"
            : isInStock
              ? "bg-near-black text-white hover:bg-ink-muted disabled:opacity-50 disabled:cursor-not-allowed"
              : "bg-gray-200 text-gray-500 cursor-not-allowed"
            }`}
        >
          {isBuyingNow ? (
      ...
      {justAdded && (
        <Link
          href="/cart"
          className="block w-full text-center py-3 border border-near-black text-near-black rounded-full uppercase tracking-widest text-sm font-medium hover:bg-near-black hover:text-white transition-colors"
        >
          View Cart
        </Link>
      )}
