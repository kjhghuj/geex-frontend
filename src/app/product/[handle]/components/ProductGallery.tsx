...
interface ProductGalleryProps {
  images: ProductImage[];
  title: string;
  thumbnail?: string | null;  // 鍟嗗搧绾у埆缂╃暐鍥句綔涓烘渶缁堝洖閫€
...
export default function ProductGallery({
  ...
  /**
   * 鍥剧墖鑾峰彇浼樺厛绾э紙涓庡悗绔?API 鏁版嵁缁撴瀯瀵归綈锛夛細
   * 
   * 鏍规嵁 API_DOCUMENTATION.md锛屽悗绔暟鎹粨鏋勫涓嬶細
   * - variant.images: 鍙樹綋涓撳睘鍥剧墖鏁扮粍 [{ id, url }]锛堢敤浜嶴KU鍒囨崲鏃舵樉绀哄搴斿浘鐗囷級
   * - product.images: 鍟嗗搧鍥剧墖鏁扮粍 [{ id, url }]
   * - product.thumbnail: 鍟嗗搧缂╃暐鍥?URL
   * 
   * 鍥剧墖鑾峰彇浼樺厛绾э細
   * 1. 棣栭€夛細鍙樹綋涓撳睘鍥剧墖 (variant.images) - 鍒囨崲SKU鏃舵樉绀哄搴斿彉浣撳浘鐗?
   * 2. 娆￠€夛細鍟嗗搧鍥剧墖鏁扮粍 (product.images) - 鍙樹綋鏃犲浘鐗囨椂鍥為€€
   * 3. 鍐嶆锛氬晢鍝佺缉鐣ュ浘 (product.thumbnail) - 鏃犲浘鐗囨暟缁勬椂鍥為€€
   * 4. 鏈€鍚庯細鍗犱綅鍥?
   */
  ...
  return (
    <div className="space-y-4">
      {/* Main Image */}
      <div
        className="relative aspect-square bg-cool-white rounded-lg overflow-hidden cursor-zoom-in"
        onMouseEnter={() => {
      ...
      >
        ...
        <div className="absolute top-4 left-4 z-10 flex flex-col gap-2">
          {isBestSeller && (
            <span className="bg-near-black text-white text-xs uppercase tracking-wider px-3 py-1.5 font-medium">
              Best Seller
            </span>
          )}
          {isNew && (
            <span className="bg-orbit-blue text-white text-xs uppercase tracking-wider px-3 py-1.5 font-medium">
              New
            </span>
          )}
          ...
          <div className="grid grid-cols-4 gap-3">
            {displayImages.slice(0, 4).map((image, idx) => (
              <button
                ...
                className={`relative aspect-square bg-cool-white rounded-lg overflow-hidden transition-all ${selectedIndex === idx
                  ? 'ring-2 ring-near-black ring-offset-2'
                  : 'hover:opacity-80'
                  }`}
              >
                <Image
            ...
            {displayImagesCount > 4 && (
              <button
                ...
                className="relative aspect-square bg-cool-white rounded-lg overflow-hidden hover:opacity-80 transition-opacity"
              >
                <Image
                  src={getImageSrc(4)}
          ...
          <div className="flex justify-center gap-2 lg:hidden">
            {displayImages.map((_, idx) => (
              <button
                ...
                onClick={() => setSelectedIndex(idx)}
                className={`w-2 h-2 rounded-full transition-colors ${selectedIndex === idx ? 'bg-near-black' : 'bg-gray-300'
                  }`}
