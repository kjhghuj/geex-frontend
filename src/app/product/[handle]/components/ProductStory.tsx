...
export default function ProductStory({ sections }: ProductStoryProps) {
    ...
    if (!sections || sections.length === 0) {
        ...
        if (process.env.NODE_ENV === "development") {
            return (
                <section className="py-24 bg-cool-white/50">
    ...
    return (
        <section className="py-24 lg:py-32 bg-cool-white/30">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                {/* Section Header */}
                <div className="text-center mb-16 lg:mb-24">
                    <span className="text-xs uppercase tracking-[0.3em] text-orbit-blue font-medium">
                        The Story
                    </span>
                    <h2 className="mt-4 font-serif text-3xl sm:text-5xl lg:text-6xl text-near-black leading-[1.15] tracking-tight">
...
function StorySectionRow({ section, isReversed }: StorySectionRowProps) {
    ...
    return (
        ...
        >
            ...
            >
                <h3 className="font-serif text-3xl sm:text-4xl lg:text-5xl text-near-black leading-tight mb-6 tracking-tight">
                    {section.title}
                </h3>
                <div className="prose prose-lg max-w-none break-words whitespace-pre-line">
                    <p className="text-ink-muted font-light text-lg md:text-xl leading-[1.8]">
                        {section.content}
                    </p>
                </div>
                {/* Decorative Line */}
                <div className="mt-8 w-16 h-[2px] bg-orbit-blue/60" />
            </div>
        </div>
    );
}
