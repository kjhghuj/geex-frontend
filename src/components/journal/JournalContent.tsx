...
const ArticleCard = ({ article }: { article: Article }) => (
    ...
    >
        <div className="relative overflow-hidden rounded-sm bg-gray-100 aspect-[3/2] mb-6">
            <div className="absolute inset-0 bg-near-black/0 group-hover:bg-near-black/5 transition-colors z-10 duration-500" />
            <ImageWithFallback
                src={article.image}
        ...
        <div className="flex flex-col flex-grow">
            <div className="flex justify-between items-center mb-3 border-b border-gray-100 pb-2">
                <span className="text-[10px] font-bold uppercase tracking-widest text-orbit-blue">
                    {article.category}
                </span>
            ...
            <h3 className="font-serif text-xl lg:text-2xl text-near-black leading-tight mb-3 group-hover:text-orbit-blue transition-colors">
                {article.title}
            </h3>
            <p className="text-sm text-ink-muted font-light line-clamp-3 leading-relaxed">
                {article.excerpt}
            </p>
        </div>
    </Link>
...
const FocusCard = ({ article }: { article: Article }) => (
    ...
    >
        ...
        <div className="w-full md:w-1/3 p-8 md:p-12 flex flex-col justify-center text-white relative">
            <span className="text-xs font-bold uppercase tracking-widest text-orbit-blue mb-4">
                {article.category}
            </span>
            <h3 className="font-serif text-2xl md:text-3xl mb-6 leading-tight group-hover:text-orbit-blue transition-colors">
                {article.title}
            </h3>
            <p className="text-gray-400 font-light text-sm leading-relaxed mb-8 border-l border-gray-600 pl-4">
                {article.excerpt}
            </p>
            <div className="flex items-center text-xs uppercase tracking-widest font-bold text-white group-hover:text-orbit-blue transition-colors mt-auto">
                Read Story{" "}
                <ArrowRight
                    size={14}
...
export default function JournalContent({
    ...
    return (
        <>
            ...
            <div className="sticky top-[104px] lg:top-[120px] z-20 bg-cool-white/95 backdrop-blur-md border-b border-gray-200 shadow-sm">
                        ...
                        {categories.map((cat) => (
                            <button
                                ...
                                className={`whitespace-nowrap px-5 py-2 lg:p-0 rounded-full lg:rounded-none border lg:border-none text-xs uppercase tracking-widest transition-all ${activeCategory === cat
                                    ? "bg-orbit-blue border-orbit-blue text-white lg:bg-transparent lg:text-near-black lg:border-b-2 lg:border-orbit-blue font-bold"
                                    : "border-gray-200 text-gray-500 hover:text-orbit-blue hover:border-orbit-blue"
                                    }`}
                            >
                                {cat}
                            </button>
                ...
                {totalPages > 1 && (
                    <div className="flex justify-center items-center gap-4 mt-16">
                        <button
                            ...
                            className="p-2 rounded-full border border-gray-200 disabled:opacity-30 hover:border-orbit-blue transition-colors"
                        >
                            <ChevronLeft size={20} />
                        </button>
                                ...
                                (page) => (
                                    <button
                                        ...
                                        className={`w-10 h-10 rounded-full text-sm font-medium transition-colors ${currentPage === page
                                            ...
                                            : "border border-gray-200 hover:border-orbit-blue"
                                            }`}
                                    >
                                        {page}
                                    </button>
